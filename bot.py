...
async def toggle_asia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer(f"{'Yoqildi' if BUTTONS_STATUS['spam_asia'] else 'O'chirildi'} Osiyo spami")
    await owner_panel_func(update, context)
...
async def toggle_ether(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer(f"{'Yoqildi' if BUTTONS_STATUS['spam_ether'] else 'O'chirildi'} Ether spami")
    await owner_panel_func(update, context)
...
async def toggle_telegram(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer(f"{'Yoqildi' if BUTTONS_STATUS['spam_telegram'] else 'O'chirildi'} Telegram spami")
    await owner_panel_func(update, context)
...
async def toggle_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer(f"{'Yoqildi' if BUTTONS_STATUS['spam_email'] else 'O'chirildi'} Gmail spami")
    await owner_panel_func(update, context)
...
async def toggle_referral(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.answer(f"{'Yoqildi' if BUTTONS_STATUS['referral'] else 'O'chirildi'} ball yig'ish")
    await owner_panel_func(update, context)
...
async def add_points_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        try:
            await context.bot.send_message(target, f"🎁 Hisobingizga {'qo\'shildi' if points > 0 else 'ayirildi'} {abs(points)} ball!\n💎 Hozirgi balansingiz: {points_after} ball")
        except:
            pass
    except:
...
def main():
    ...
    print("=" * 60)
    print("🌟 Bot xizmatlari - 100% ishlamoqda 🌟")
    print("✅ Barcha xizmatlar faol")
    print("✅ Ball tizimi faol")
    print("✅ Tugmalar rangli va chiroyli")
    print("✅ Admin paneli faol")
    print("✅ Egasi paneli faol")
    print("✅ Umumiy va shaxsiy xabar yuborish faol")
    print("✅ Ball o'tkazish faol")
    print("✅ Havola statistikasi faol")
    print("✅ Kanal qo'shish va tekshirish faol")
